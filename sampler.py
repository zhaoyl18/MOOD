...
from evaluate import evaluate, evaluate_qed_sa
...
class Sampler_DPS(object):
    ...
    def sample(self):
        ...
        str_time = time.strftime('%b%d-%H:%M:%S', time.gmtime())
        
        self.log_name = f"{self.config.model.diff.ckpt}_{self.config.model.prop.ckpt}_weightx={self.config.model.prop.weight_x}_{str_time}"
        print(f"logname: {self.log_name}_{str_time}")
        ...
        result = evaluate_qed_sa(os.path.join(self.log_dir, self.log_name), gen_smiles, gen_mols)
        
        logger.log('**********************************************')
        logger.log(f'QED: '
                f'{result["qed-mean"]:.2f} ± {result["qed-std"]:.2f}')
        logger.log(f'SA: '
                f'{result["sa-mean"]:.2f} ± {result["sa-std"]:.2f}')
        logger.log('**********************************************')
